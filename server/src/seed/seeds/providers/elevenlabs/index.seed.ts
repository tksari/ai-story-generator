...
export async function seedElevenLabs(prisma: PrismaClient) {
  try {
    const dir = __dirname;

    const provider = loadJsonFile<Provider>(`${dir}/data/provider.json`);
    ...
    console.log("ElevenLabs seeding completed");
  } catch (error) {
    console.error("Error seeding ElevenLabs:", error);
    throw error;
  }
}
